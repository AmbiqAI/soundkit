    ...
    for step, batch in enumerate(dataset):
        ...
        if 1:
            if step % 100 == 0:
                ...
                spec_sn = tf.signal.rfft(feat_sn, [stft_feat["fft_size"]])
                pspec_sn = 20 * tf_log10_eps(tf.abs(spec_sn))[idx].numpy()

                fig = plot_spectrograms(
                    images=[pspec_sn.T],
                    titles=[ "noisy logspec"],
                    vmin_vmax=[(-80, 10)],
                    show_colorbar=True,
                    show_fig=False       # set False if only saving
                )
                pred_vad = tf.math.softmax(pred, axis=-1).numpy()[idx][..., 1]

                plt.plot(pred_vad*200, label='predicted VAD')
