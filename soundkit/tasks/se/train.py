...
def run_epoch(
    ...
    Args:
        config (dict): Configuration dictionary containing:
            ...
            - total_batches: dict with 'train' and 'test' counts
        dataset (tf.data.Dataset): The dataset to iterate over.
        training (bool): Whether this is a training epoch (True) or testing (False).
    ...
    total_batches = config["total_batches"]['train'] if training else config["total_batches"]['test']

    total_steps=total_batches * epoch

    train_summary_writer = config["train_summary_writer"]
    train_tag = "train" if training else "test"
    ...
    for step, batch in enumerate(dataset):
        ...
        # Perform one training or test step
...
def train(params: SKTaskParams):
    ...
    ds_test, batches_test = create_dataset(
        tfrecord_list['test'],
        batchsize=batchsize,
        is_shuffle=False,
    )
    ...
    train_log = load_train_log(log_path)
    ...
    for epoch in range(epoch_loaded_1, params_train['epochs']):
        ...
        train_config={
            ...
            'optimizer': optimizer,
            'loss_fn': loss_fn,
            'total_batches': {'train': batches_train, 'test': batches_test},
        ...
        log_epoch["test_loss"] = float(loss.result().numpy())
        if train_summary_writer is not None:
            with train_summary_writer.as_default():
                tf.summary.scalar(f"test/loss", loss.result(), step=epoch)
