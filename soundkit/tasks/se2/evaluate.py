...
def evaluate(params: SKTaskParams):
    ...
    buffer_sn = LookaheadBuffer(
        ...
        feature_dim=feat_params["frame_size"],
        batchsize=batchsize,
        dtype=tf.float32,)
    ...
    for step, batch in enumerate(dataset):
        ...
        if 0:
            ...
            if feat_sn_norm.dtype == tf.complex64:
                # If complex input, convert to real and imaginary parts
                feat_sn_norm = tf.stack(
                    [tf.math.real(feat_sn_norm), tf.math.imag(feat_sn_norm)],
                    axis=-1)
            audio_en_frames = model(feat_sn_norm, training=False)

        audio_en = frame_class.reconstruct(
            audio_en_frames)

        spec_sn_delay = tf.signal.rfft(
            input_sn_delay,
            [feat_params["fft_size"]])
        spec_en_delay = tf.signal.rfft(
            audio_en_frames,
            [feat_params["fft_size"]])
        ...
        if step < 10:
            ...
            plot_spectrograms(
                images=[
                    20 * tf_log10_eps(tf.abs(spec_sn_delay).numpy()[0].T),
                    20 * tf_log10_eps(tf.abs(spec_en_delay).numpy()[0].T),
                ],
                titles=["Noisy logspec", "Enhanced logspec"],
