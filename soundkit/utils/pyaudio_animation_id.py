...
class AudioShowClass:
    def __init__(
        ...
        self.register={
            ...
            'name': None,
            'counts': 0}
        axbox = plt.axes([0.35, 0.17, 0.3, 0.05])  # Adjust as needed
        self.text_box_name = TextBox(
            axbox,
            f'Name:{self.register["name"]}',
            initial=self.register['name'])
        self.text_box_name.label.set_fontsize(12)

        ax_reg_button = plt.axes([0.67, 0.17, 0.12, 0.05])  # Position next to the box
        self.button_register = Button(ax_reg_button, 'Register', color='w', hovercolor='lightblue')
        self.button_register.label.set_fontsize(14)
        self.button_register.on_clicked(self.callback_register)
        self.register_text = self.fig.text(0.02, 0.95, "", fontsize=14, weight='bold', color='darkblue')
        self.counts_registered=-1
        self.id_label = self.fig.text(
            0.7, 0.95,
            "Identified: N/A", fontsize=14, color='darkgreen')
        plt.show()

    def callback_register(self, event):
        if self.text_box_name.text.strip():
            self.register['is_register'] = True
            self.register['name'] = self.text_box_name.text.strip()
            # Trigger the record button after registration
            
            self.callback_record(event)  # You can also pass None if you don't need the event
    ...
    def callback_record(self, event):
        ...
        if self.register['is_register']:
            self.register_text.set_text(f"Mode: registration")
        else:
            self.register_text.set_text(f"Mode: identification")
            self.text_box_name.set_val("")
        ...
        if self.lock_button == 0:
            ...
            def callback_streamin(in_data, frame_count, time_info, status):
                ...
                if self.proc_st:
                    ...
                    processed, request = self.proc_st(data_fr, register=self.register)

                    # update the register status and counts
                    if request['is_register'] != self.register['is_register']:
                        self.register['is_register'] = request['is_register']
                    ...
                    if self.register['is_register']:
                        if request['counts'] != self.register['counts']:
                            self.register['counts'] = request['counts']
                            self.register_text.set_text(f"Mode: registration, {self.register['counts']}")

                    self.data_buffer[start:start + self.frame_size, 1] = processed[:, 0]
                    self.data_trigger[start:start + self.frame_size, 1] = processed[:, 0]

                    if self.register['is_register'] is False:
                        if request['info']: # this will ignore {}
                            self.id_label.set_text(f"Identified: {request['info']}")
