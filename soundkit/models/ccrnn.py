...
class CCRNN(tf.keras.Model):
    ...
    def __init__(
            self,
            params: CCRNNParams = CCRNNParams(),
            **kwargs):
        super().__init__()
        self.params = params
        self.layer_stack = []
        ...
        for i, layer_def in enumerate(params.layer_configs):
            if layer_def['type'] == 'conv2d':
            ...
            elif layer_def['type'] == 'fc_complex':
                self.states.append(tf.constant(0, dtype=tf.float32))
                ...
                self.layer_stack.append([real, imag])
    ...
    def call(self, x, mask = 1.0, training=False):
        ...
        for layer, config, state in zip(self.layer_stack, self.params.layer_configs, self.states):
            if config['type'] == 'conv2d':
                ...
                x = layer(x, training=training)
            elif config['type'] == 'fc_complex':
                real, imag = layer
                x_real = x[...,0]
                x_imag = x[...,1]
                x_real = real(x_real, training=training)
                x_imag = imag(x_imag, training=training)
                
                x = tf.stack([x_real, x_imag], axis=-1)
        return x
    def get_former_neurons(self, params, current_layer_index):
        ...
        if current_layer_index == 0:
            ...
            for layer_config in reversed(params.layer_configs[:current_layer_index]):
                if layer_config['type'] in ['lstm', 'fc', 'fc_complex']:
