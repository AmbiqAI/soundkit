...
class CCRNN(tf.keras.Model):
    """Convolutional Recurrent Neural Network (CCRNN) for sequence prediction.
    ...
    def __init__(
        ...
        for i, layer_def in enumerate(params.layer_configs):
            if layer_def['type'] == 'conv2d':
                ...
                if k_t > 1:
                    state = tf.Variable(
                        tf.zeros((params.batchsize, k_t - 1, input_dim, 2), dtype=tf.float32),
                        trainable=False,
    ...
    def call(
        ...
        for layer, config in zip(self.layers_list, self.params.layer_configs):
            if config['type'] == 'conv2d': # 1d conv
                ...
                x = layer(x, training=training)
        x = tf.reshape(x, [self.params.batchsize, -1, self.params.dim_feat, 2])
        return x
