...
int NNSPClass_init(
    ...
    FeatureClass_construct(
        ...
        pt_params->num_frames_infer,
        pt_params->pt_stft_win_coeff,
        pt_params->p_melBanks, pt_params->feature_type);
...
#else
        for (int i = 0; i < 160; i++) {
            int32_t temp = ((int32_t)rawPCM[i] * (int32_t)pt_inst->pt_params->pre_gain_q1) >> 1;

            rawPCM[i] = (int16_t) MAX(MIN(temp, MAX_INT16_T), MIN_INT16_T);
        }
#endif
...
void se_post_proc_cmplx(
    ...
    for (int i = start_bin; i < start_bin + nn_dim_out; i++) { // TODO: check
        ...
        int64_t real = (int32_t) ((m_r * x_r - m_i * x_i) >> 15);

        spec[2 * i] = (int32_t) MAX(MIN(real, MAX_INT32_T), MIN_INT32_T);

        int64_t imag = (int32_t) ((m_r * x_i + m_i * x_r) >> 15);
        
        spec[2 * i + 1] = (int32_t) MAX(MIN(imag, MAX_INT32_T), MIN_INT32_T);
